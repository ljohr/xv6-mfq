...
struct proc *mfq[4][NPROC];
int end_index[4] = {0, 0, 0 ,0};
int time_quantum[4] = {16, 8, 4, 2};
...
scheduler(void)
{
 // struct proc *p;
  ...
  for(;;){
    ...
    for (int p = 3; p >=0; p--) { 
      for (int i = 0; i < end_index[p]; i++) {
          found = 1;
          struct proc *my_proc = mfq[p][i]; 

          acquire(&my_proc->lock);
          my_proc->state = RUNNING;
          c->proc = my_proc;
          
          swtch(&c->context, &my_proc->context);

          c->proc = 0;
          release(&my_proc->lock);
          
          // move to a lower priority
          if (my_proc->ticks >= time_quantum[p]) {
            dequeue(my_proc);
            if (my_proc->prio > 0) {
              acquire(&my_proc->lock);
              my_proc->prio--;
...
procinfo(uint64 proc_array, int max_procs)
{
  ...
  for (cur_proc=proc; cur_proc < &proc[NPROC]; cur_proc++)
  {
    ...
    cur_uproc.prio = cur_proc->prio;
    cur_uproc.ticks = cur_proc->ticks;
    cur_uproc.block_count = cur_proc->block_count;
    cur_uproc.full_count = cur_proc->full_count;
    release(&cur_proc->lock);
...
dequeue(struct proc *p) {
  p->ticks = 0;
  int cur_prio = p->prio;
  for (int i = 0; i < end_index[cur_prio]; i++) {
    if (mfq[cur_prio][i] == p){
      for (int j = i; j < end_index[p->prio]; j++) {
        mfq[cur_prio][j] = mfq[p->prio][j+1];    
      }
      break;
    }
  }
